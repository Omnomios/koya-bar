...
export class NetworkManager
{
            ...
            const decodeSsid = (ssid) => {
                ...
                try
                {
                    ...
                    for(let i = 0; i < trimmed.length; )
                    {
                        ...
                        if((b0 & 0xE0) === 0xC0 && i < trimmed.length)
                        {
                            const b1 = trimmed[i++] & 0x3F;
                            const cp = ((b0 & 0x1F) << 6) | b1;
                            out += String.fromCharCode(cp);
                            continue;
                        }
                        if((b0 & 0xF0) === 0xE0 && i + 1 < trimmed.length)
                        {
                            const b1 = trimmed[i++] & 0x3F;
                            const b2 = trimmed[i++] & 0x3F;
                            const cp = ((b0 & 0x0F) << 12) | (b1 << 6) | b2;
                            out += String.fromCharCode(cp);
                            continue;
                        }
                        if((b0 & 0xF8) === 0xF0 && i + 2 < trimmed.length)
                        {
                            const b1 = trimmed[i++] & 0x3F;
                            const b2 = trimmed[i++] & 0x3F;
                            const b3 = trimmed[i++] & 0x3F;
                        ...
                        // Invalid sequence: fallback to latin1 byte
                        out += String.fromCharCode(b0);
                    }
    ...
    // Create a connection using nmcli via the Process plugin.
    // Supports type: 'wifi' or 'gsm'. Returns { ok, uuid, name, stdout, stderr }.
    async createConnection(options)
    {
        await this.connect();
        const type = options && typeof options.type === 'string' ? options.type.toLowerCase() : '';
        if(type === 'wifi') return await this._createWifiConnectionDbus(options);
        if(type === 'gsm') return await this._createGsmConnectionDbus(options);
        throw new Error('createConnection: options.type must be "wifi" or "gsm"');
    }

    async _createWifiConnectionDbus(opts)
    {
        ...
        const settings = {
            ...
            '802-11-wireless': {
                'ssid': { _t: 'ay', _v: toUtf8Bytes(ssid) },
                ...(hidden ? { 'hidden': true } : {}),
                'mode': 'infrastructure'
            },
            ...(password ? {
        ...
        // Find device path to activate
        const devicePath = await this._findDevicePath({ ifname, typeNum: 2 }); // wifi device type = 2

        // Try AddAndActivateConnection first
        try
        {
            ...
            // We may want to query back the UUID
            const all = await this.getAllConnections(false);
            const created = all.find((c) => c?.id === name);
            return { ok: true, uuid: created?.uuid, name, stdout: '', stderr: '' };
        }
        catch(_)
        {
            // Fallback: AddConnection then ActivateConnection
            try
            {
                ...
                const devPath = devicePath || await this._findDevicePath({ ifname, typeNum: 2 });
                const active = await DBus.callComplex(
                    'org.freedesktop.NetworkManager',
                    '/org/freedesktop/NetworkManager',
                    'org.freedesktop.NetworkManager',
                    'ActivateConnection',
            ...
            {
                return { ok: false, uuid: undefined, name, stdout: '', stderr: String(err2 || '') };
            }
        }
    }

    async _createGsmConnectionDbus(opts)
    {
        ...
        const settings = {
            ...
            'gsm': {
                ...
                ...(pin ? { 'pin': pin, 'pin-flags': 0 } : {}),
            },
            'ipv4': { 'method': 'auto' },
            'ipv6': { 'method': 'ignore' }
        };

        const devicePath = await this._findDevicePath({ ifname, typeNum: 8 }); // modem device type = 8
        ...
        catch(_)
        {
            try
            {
                ...
                const devPath = devicePath || await this._findDevicePath({ ifname, typeNum: 8 });
                const active = await DBus.callComplex(
                    'org.freedesktop.NetworkManager',
                    '/org/freedesktop/NetworkManager',
                    'org.freedesktop.NetworkManager',
    ...
    async _findDevicePath({ ifname, typeNum })
    {
        const parseObject = (value) =>
        {
            if(value && typeof value === 'object') return value;
        ...
        try
        {
            ...
            for(const p of devicePaths)
            {
                ...
                if(!ifname && typeof dType === 'number' && dType === typeNum && !best) best = p;
            }
            return best;
        }
        catch(_)
